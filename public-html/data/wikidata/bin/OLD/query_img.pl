...
if (! -f "$DIR_PERSON/$WIKIDATA_ENTRY") {
    system "spang wikidata http://spang.dbcls.jp/library/wikidata/person_img wd:$WIKIDATA_ENTRY -a > $DIR_PERSON/$WIKIDATA_ENTRY";
}
...
sub get_children {
	...
	while (<FILE>) {
		...
		if (! -f "$DIR_CHILD/$wikidata_entry") {
		    system "spang wikidata http://spang.dbcls.jp/library/wikidata/child_img $f[0] -a > $DIR_CHILD/$wikidata_entry";
		    $count ++;
		}
	    } else {
...
sub get_parents {
	...
	while (<FILE>) {
		...
		if (! -f "$out_dir/$wikidata_entry") {
		    system "spang wikidata http://spang.dbcls.jp/library/wikidata/parent_img $f[0] -a > $out_dir/$wikidata_entry";
		    $count ++;
		}
	    } else {
		print STDERR "WARNING: $f[0] is not wikidata entry\n";
