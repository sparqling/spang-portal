...
if ($KEY_VAL{title} and $KEY_VAL{dirname}) {
    print " >> <a href='/library/$KEY_VAL{dirname}/'>$KEY_VAL{title}</a>";
}
print " >> schema";
...
for my $file (@file) {
    if ($file =~ /^(go|sio|so|pato|po|to|eco|agrold).ttl.pdf$/) {
	next;
    }
    if ($file =~ /~$/) {
	next;
...
sub print_link_to_file {
    ...
    print "<a href='$file' class='link_title'>$name</a>";
    print "<br>";
    print "<a href='$file' class='link_title'><img src='$png'></a>";
}
